...
export type SetChoice = {
  setId: number;
  name: string;
  /** The set's art, resolved on the server: a flower, or the slot being filled. */
  icon: string | null;
  /** One line per bonus, already formatted. */
  effects: string[];
  /** Pieces of it on offer. Zero still shows, dimmed, after the rest. */
  count: number;
};
...
export function SetStripView({
  ...
  /** The set picked, or `null` when the picked one is picked again. */
  onSelect: (setId: number | null) => void;
  /** Dims the strip while whatever it filters is being worked out. */
  pending?: boolean;
  /**
   * The chosen set's bonus under the strip. Off in the gear dialog, where
   * height is the scarce thing and a card's set art opens the same text.
   */
  showEffects?: boolean;
}) {
  const t = useTranslations('artifacts');
  const active = sets.find((set) => set.setId === selected) ?? null;
